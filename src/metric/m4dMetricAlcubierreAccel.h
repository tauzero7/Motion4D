...
/*!  \class  m4d::MetricAlcubierreAccel
     \brief  Alcubierre warp metric in Cartesian coordinates (t,x,y,z).
            ...
            \f[ f(r_s)=\frac{\tanh(\sigma(r_s+R))-\tanh(\sigma(r_s-R))}{2\tanh(\sigma R)}. \f]
             ...
             In contrast to the standard Alcubierre metric, the velocity of the warp
             bubble here is not constant.

             The default local tetrad is the comoving one
             (enum_nat_tetrad_default = enum_nat_tetrad_lnrf).
*/
// --------------------------------------------------------------------------------
#ifndef M4D_METRIC_ALCUBIERRE_ACCEL_H
#define M4D_METRIC_ALCUBIERRE_ACCEL_H
...
// ---------------------------------------------------
//    class definition:   MetricAlcubierreAccel
// ---------------------------------------------------
class MetricAlcubierreAccel : public Metric {
public:
    MetricAlcubierreAccel(double sigma = 1.0, double R = 1.0, double x0 = 0.0, double v0 = 0.0, double alpha = 0.0, double t0 = 0.0);
    virtual ~MetricAlcubierreAccel();
