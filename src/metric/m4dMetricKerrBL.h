 ...
 * @brief  Kerr metric in spherical Boyer-Lindquist coordinates (t,r,theta,phi).
    ...
    with \f$\Delta = r^2-2mr+a^2\f$  and  \f$\Sigma = r^2+a^2 \cos(\vartheta)^2\f$. 'm' is
                                                                                  the mass of the black hole and 'a' the
 angular momentum per unit rest mass.

        The natural local tetrad of the Kerr spacetime is given by
             \f[ \mathbf{e}_{(0)} = \Gamma\left(\partial_t+\zeta\partial_{\varphi}\right),\quad
 \mathbf{e}_{(1)}=\sqrt{\frac{\Delta}{\Sigma}}\partial_r,\quad
 \mathbf{e}_{(2)}=\frac{1}{\sqrt{\Sigma}}\partial_{\vartheta},\quad \mathbf{e}_{(3)} =
 \Gamma\left(\pm\frac{g_{t\varphi}+\zeta g_{\varphi\varphi}}{\sqrt{\Delta}\sin\vartheta}\partial_t\mp\frac{g_{tt}+\zeta
 g_{t\varphi}}{\sqrt{\Delta}\sin\vartheta}\partial_{\varphi}\right),\f] with the free parameter \f$\zeta\f$. In this
 metric, we use the following two entities: \f[ \omega = \frac{2mar}{\Sigma\left(r^2+a^2\right)+2ma^2r\sin^2\vartheta}
 \f] and \f[ \Gamma^{-2} =
 \left(1-\frac{2mr}{\Sigma}\right)+\frac{2mar\sin^2\vartheta}{\Sigma}\zeta-\left(r^2+a^2+\frac{2ma^2r\sin^2\vartheta}{\Sigma}\right)\zeta^2\sin^2\vartheta.
 \f]
    ...
    This metric is discussed e.g. in <br><br>
        ...
        Bardeen et al,<br><b>"Rotating black holes: Locally nonrotating frames, energy extraction, and scalar
 synchrotron radiation,"</b><br>The Astrophysical Journal <b>178</b>, 347--359 (1972).<br>
