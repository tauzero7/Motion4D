/**
 * @file    m4dMetricPravda_C_CanI.h
 * @author  Felix Beslmeisl
 *
 * @brief  This is the C-Metric as given in class MetricPTD_C in "canonical coordinates adapted to the boost-rotation
 symmetry".

     The line element is given by

     \f[ds^2 = \frac{1}{z^2-t^2}\left(e^{\rho}r^2(z\,dt-t\,dz)^2 -e^{\lambda}(z\,dz-t\,dt)^2 \right) -
 e^{\lambda}\,dr^2-r^2e^{-\rho}\,d\varphi^2\f] with
 \f[e^{\rho}=\frac{R_3+R+Z_3-r^2}{4\alpha^2\left(R_1+R+Z_1-r^2\right)},\f]
          \f[e^{\lambda}=\frac{2\alpha^2\left(R(R+R_1+Z_1)-Z_1r^2\right) \left(R_1R_3+(R+Z_1)(R+Z_3)-(Z_1+Z_3)r^2)
 \right)}{R_iR_3\left( R(R +R_3+Z_3) - Z_3r^2\right)},\f] \f[R=\frac12\left(z^2-t^2+r^2\right),\f] \f[R_i=\sqrt{(R +
 Z_i)^2 - 2Z_i r^2},\f] \f[Z_i=z_i-z_2,\f] \f[\alpha^2=\frac14 \frac{m^2}{A^6(z_2-z_1)^2(z3-z1)^2},\f]
     ...
     \f[  \mathbf{e}_{(t)} = \frac{1}{\sqrt{z^2-t^2}}\left( qze^{-\rho/2}\,\partial_t, +
 te^{-\lambda/2}\,\partial_z,\right)\quad \mathbf{e}_{(r)} = e^{-\lambda/2}\,\partial_r, \quad \mathbf{e}_{(\varphi)} =
 re^{\rho/2}\,\partial_{\varphi}, \quad \mathbf{e}_{(z)} = \frac{1}{\sqrt{z^2-t^2}}\left( qte^{-\rho/2}\,\partial_t, +
 ze^{-\lambda/2}\,\partial_z,\right)\f]

     and for \f$z^2-t^2<0\f$ by
     \f[  \mathbf{e}_{(t)} = \frac{1}{\sqrt{z^2-t^2}}\left( qte^{-\rho/2}\,\partial_t, +
 ze^{-\lambda/2}\,\partial_z,\right) \quad \mathbf{e}_{(r)} = e^{-\lambda/2}\,\partial_r, \quad \mathbf{e}_{(\varphi)} =
 re^{\rho/2}\,\partial_{\varphi}, \quad \mathbf{e}_{(z)} = \frac{1}{\sqrt{z^2-t^2}}\left( qze^{-\rho/2}\,\partial_t, +
 te^{-\lambda/2}\,\partial_z,\right).\f]
