...
bool MetricPravda_C_Can::calculateChristoffels(const double* pos)
{
    ...
    double t115 = -t13 * t14 * t6 - t82 * t2 * t9 * t4 + 2.0 * t88 - t13 * t90 * t9 + t95 + t98 - t99 * t43 + 2.0 * t103
        - t30 * t26 * tau * t42 + 2.0 * t1 * t11 * t42 * t4 - 2.0 * t49 * t42 - t47 * t53;
    double t119 = t1 * t71;
    double t121 = t21 * t11;
    double t125 = t63 * t76;
    ...
    double t256 = t9 * t44;
    double t264 = -t99 * t6 - 2.0 * t9 * t49 + 2.0 * t19 + 2.0 * t9 * t1 * t30 - t256 * t147 - t256 * t82 + t36 + t32
        - t43 * t15 - t43 * t18 + 2.0 * t46 - t54 * t12 * t2;
    double t281 = -t30 * t44 * t6 - t142 + t88 - t145 + 2.0 * t95 - t153 + t149 + 2.0 * t98 - t150 + t103 - t156
        - t173 * t14 * t4;
...
bool MetricPravda_C_Can::setParam(const char* pName, double val)
{
    ...
    Z1 = z_i[0] - z_i[1];
    Z3 = z_i[2] - z_i[1];
    alpha2 = 0.25 * m * m / A / A / A / A / A / A / (z_i[1] - z_i[0]) / (z_i[1] - z_i[0]) / (z_i[2] - z_i[0])
        / (z_i[2] - z_i[0]);
    q = 0.25 / alpha2;
    return true;
}
...
double MetricPravda_C_Can::rho_tau(double tau, double zeta, double eta)
{
    ...
    double t33 = t27 * t27;
    return (-1 / t11 * tau - tau) * t16 / t27 / 4.0
        - (t11 / 2.0 + t24 - t25 - t26 + Z3) * t16 / t33 * (-1 / t22 * tau - tau) / 4.0;
}
...
double MetricPravda_C_Can::rho_zeta(double tau, double zeta, double eta)
{
    ...
    double t27 = t22 / 2.0 + t24 - t25 - t26 + Z1;
    double t33 = t27 * t27;
    return (1 / t11 * zeta + zeta) * t16 / t27 / 4.0
        - (t11 / 2.0 + t24 - t25 - t26 + Z3) * t16 / t33 * (1 / t22 * zeta + zeta) / 4.0;
}
...
double MetricPravda_C_Can::rho_eta(double tau, double zeta, double eta)
{
    ...
    double t38 = t32 * t32;
    return (1 / t11 * (t13 - 16.0 * Z3 * eta) / 4.0 - eta) * t21 / t32 / 4.0
        - (t11 / 2.0 + t29 - t30 - t31 + Z3) * t21 / t38 * (1 / t27 * (t13 - 16.0 * Z1 * eta) / 4.0 - eta) / 4.0;
}
...
double MetricPravda_C_Can::lambda(double tau, double zeta, double eta)
{
    ...
    return 8.0 * t1 * (t5 * (t6 - t7 + t8 + t16 / 2.0 + Z1) / 2.0 - t13)
        * (t16 * t26 / 4.0 + (t6 - t7 + t8 + Z3) * (t6 - t7 + t8 + Z1) - (Z1 + Z3) * t4) / t16 / t26
        / (t5 * (t6 - t7 + t8 + t26 / 2.0 + Z3) / 2.0 - t23);
}
 ...
 * \param eta : r-coordinate
 */
double MetricPravda_C_Can::lambda_tau(double tau, double zeta, double eta)
{ // diff(lambda(tau,zeta,eta),tau)
    double t1 = alpha2;
    double t2 = zeta * zeta;
    double t3 = t2 / 2.0;
    double t4 = tau * tau;
    ...
    double t69 = t46 * tau;
    double t79 = t45 * t45;
    return 8.0 * t1 * (-tau * t17 + t19 * (-t20 * tau - tau) / 2.0) * t38 * t47
        + 8.0 * t52 * (-t20 * t30 * tau / 2.0 - t15 * t40 * tau / 2.0 - tau * t34 - t33 * tau) * t47
        + 16.0 * t65 / t15 / t14 * t40 * t69 + 16.0 * t65 * t20 / t30 / t29 * t69
        - 8.0 * t65 * t41 / t79 * (-tau * t43 + t19 * (-t40 * tau - tau) / 2.0);
}
...
double MetricPravda_C_Can::lambda_zeta(double tau, double zeta, double eta)
{
    ...
    double t69 = t46 * zeta;
    double t79 = t45 * t45;
    return 8.0 * t1 * (zeta * t17 + t19 * (t20 * zeta + zeta) / 2.0) * t38 * t47
        + 8.0 * t52 * (t20 * t30 * zeta / 2.0 + t15 * t40 * zeta / 2.0 + zeta * t34 + t33 * zeta) * t47
        - 16.0 * t65 / t15 / t14 * t40 * t69 - 16.0 * t65 * t20 / t30 / t29 * t69
        - 8.0 * t65 * t41 / t79 * (zeta * t43 + t19 * (t40 * zeta + zeta) / 2.0);
}
...
double MetricPravda_C_Can::lambda_eta(double tau, double zeta, double eta)
{
    ...
    double t76 = t58 * t44;
    double t91 = t51 * t51;
    return 8.0 * t1 * (eta * t17 + t19 * (eta + t20 * t24 / 4.0) / 2.0 - 2.0 * t22) * t44 * t53
        + 8.0 * t58 * (t20 * t36 * t24 / 8.0 + t15 * t46 * t65 / 8.0 + eta * t40 + t39 * eta - 2.0 * t42 * eta) * t53
        - 4.0 * t76 / t15 / t14 * t46 * t52 * t24 - 4.0 * t76 * t20 / t36 / t35 * t52 * t65
        - 8.0 * t76 * t47 / t91 * (eta * t49 + t19 * (eta + t46 * t65 / 4.0) / 2.0 - 2.0 * t63);
}
