...
int main( int argc, char* argv[] )
{
  ...
  std::cerr << m4d::stl_break_condition[geod->calcParTransport(mObject.startPos,coDir,
               e0,e1,e2,e3,
               mObject.maxNumPoints, mObject.points, mObject.dirs, mObject.lambda,
	          base[0], base[1], base[2], base[3]) ] << std::endl;
  ...
  {
    fprintf(stdout,"%5d  %12.8f %12.8f %12.8f %12.8f  ",i,mObject.points[i][0],mObject.points[i][1],mObject.points[i][2],mObject.points[i][3]);
    for(int j=0; j<4; j++)
    {
#if 1            
      fprintf(stdout,"%12.8f %12.8f %12.8f %12.8f  ",base[j][i][0],base[j][i][1],base[j][i][2],base[j][i][3]);
#else
